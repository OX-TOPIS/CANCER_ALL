<template>
  <div class="row g-0 text-center">
    ...
    <div
      ...
      style="min-height: calc(100vh - 63px)"
    >
      <div class="border rounded shadow p-3" style="margin: 50px; width: 60%">
        <h3 style="margin-bottom: 30px">นำเข้าข้อมูลผู้ป่วย</h3>
        <hr style="border: none; border-top: 2px solid #000; width: 100%" />
        <div class="d-flex justify-content-between">
          <h5>เลือกไฟล์ข้อมูลผู้ป่วย จากนั้นคลิก "นำเข้าข้อมูลผู้ป่วย"</h5>
          <div class="d-flex">
      ...
      <div class="border rounded shadow p-3" style="margin: 50px; width: 60%">
        <h3 style="margin-bottom: 30px">ส่งออกข้อมูลผู้ป่วย</h3>
        <hr style="border: none; border-top: 2px solid #000; width: 100%" />
        <div class="d-flex justify-content-between">
          <h5>คลิก "ส่งออกข้อมูลผู้ป่วย" จากนั้นเลือกข้อมูลที่ต้องการส่งออก</h5>
          <button
            class="btn btn-success text-white"
            type="button"
...
export default {
  ...
  methods: {
    async showExportPopup() {
      const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
          ...
          if (document.getElementById("HN").checked)
            selectedOptions.push("treatment.HN AS HN");
          if (document.getElementById("FullName").checked)
            selectedOptions.push("User.firstName AS firstName");
          if (document.getElementById("FullName").checked)
            selectedOptions.push("User.lastName AS lastName");
          if (document.getElementById("IDcard").checked)
            selectedOptions.push("User.userName AS IDcard");
          if (document.getElementById("Cancer").checked)
            selectedOptions.push("cancerType");
          if (document.getElementById("CancerState").checked)
            selectedOptions.push("cancer_patient.cancerState AS cancerState");
          if (document.getElementById("Disease").checked)
            selectedOptions.push("disease");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.fatherName AS fatherName");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.motherName AS motherName");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.contactPerson AS contactPerson");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.spouseName AS spouseName");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.relatedAs AS relatedAs");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.IDcardAddress AS IDcardAddress");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.currentAddress AS currentAddress");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.contactAddress AS contactAddress");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.nationality AS nationality");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.occupation AS occupation");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.education AS education");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.marriageStatus AS marriageStatus");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.bloodGroup AS bloodGroup");
          if (document.getElementById("History").checked)
            selectedOptions.push("history.religious AS religious");
      ...
      if (formValues) {
        // ส่งค่าที่เลือกไปยัง exportCSV
        this.exportCSV(formValues);
    ...
    //     window.location.href = `https://cancer-api.gadoz.dev/export/csv?${queryParams}`;
    //     console.log(req.query.fields);
    // },
    exportCSV(formValues) {
      // ตรวจสอบว่ามีค่าที่ส่งเข้ามาหรือไม่
      if (
        !formValues ||
        !Array.isArray(formValues) ||
        formValues.length === 0
      ) {
        console.error("formValues ต้องเป็น array และต้องไม่ว่าง");
        return;
      }

      // สร้าง query string จาก formValues
      const queryParams = formValues
        .map((option) => `fields=${encodeURIComponent(option)}`)
        .join("&");

      // ตรวจสอบ queryParams ก่อนเปลี่ยนหน้า
      if (queryParams) {
        const url = `https://cancer-api.gadoz.dev/export/csv?${queryParams}`;
        console.log("กำลังส่งไปยัง URL:", url); // ช่วย Debug URL
        window.location.href = url;
      } else {
        console.error("Query Params ว่างเปล่า");
      }
    },
