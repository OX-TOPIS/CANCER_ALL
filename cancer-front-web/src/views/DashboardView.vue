            ...
            <ul class="dropdown-menu">
              <li>
                <label class="dropdown-item">
                  <input
                    type="checkbox"
                    v-model="selectAllCancer"
                    @change="toggleAllCancer"
                  />
                  เลือกทั้งหมด
                </label>
              </li>
            ...
            <ul class="dropdown-menu">
              <li>
                <label class="dropdown-item">
                  <input
                    type="checkbox"
                    v-model="selectAllRangeage"
                    @change="toggleAllRangeage"
                  />
                  เลือกทั้งหมด
                </label>
              </li>
            ...
            <ul class="dropdown-menu">
              <li>
                <label class="dropdown-item">
                  <input
                    type="checkbox"
                    v-model="selectAllFeedback"
                    @change="toggleAllFeedback"
                  />
                  เลือกทั้งหมด
                </label>
              </li>
            ...
            <ul class="dropdown-menu">
              <li>
                <label class="dropdown-item">
                  <input
                    type="checkbox"
                    v-model="selectAllFomula"
                    @change="toggleAllFomula"
                  />
                  เลือกทั้งหมด
                </label>
              </li>
...
export default {
  ...
  data() {
    return {
      ...
      feedbacks: [
        {
          label: "กดการทำงานของไขกระดูก หรือภูมิต้านทานต่ำ",
        ...
        {
          label: "อ่อนเพลีย / ครั่นเนื้อครั่นตัว",
  ...
  methods: {
    ...
    async fetchAgeGroups() {
      try {
        const response = await axios.get(
          "https://cancer-api.gadoz.dev/age-groups"
        );
    ...
    async fetchCancerData() {
      try {
        const response = await axios.get(
          `https://cancer-api.gadoz.dev/cancerstate-cancer-summary?${this.StringState}`
        );
    ...
    async fetchCancerStateData() {
      try {
        const response = await axios.get(
          "https://cancer-api.gadoz.dev/cancerstate-cancer-summary?cancerState=1,2,3,4"
        );
    ...
    async fetchFeedbackData() {
      try {
        const response = await axios.get(
          "https://cancer-api.gadoz.dev/feedback-summary"
        );
        this.feedbackSummary = response.data;

        console.log("feedback summary", this.feedbackSummary);
        console.log(
          "this.feedbackSummary[0][0]",
          this.feedbackSummary["มะเร็งต่อมไทรอยด์"]["ผมร่วง/ ผมบาง"]
        );
      } catch (error) {
    ...
    async fetchFomulaData() {
      try {
        const response = await axios.get(
          "https://cancer-api.gadoz.dev/fomula-summary"
        );
  ...
  computed: {
    ...
    selectedCancerstates() {
      return this.cancerstates
        .filter((cancerstate) =>
          this.showCancerstate.includes(cancerstate.value)
        )
    ...
    chartData2() {
      ...
      const selectedAgeKeys = this.selectedRangeages
        .map((range) => {
          switch (range) {
            case "อายุ 0-18":
              return "0-18";
      ...
      if (this.selectedGenders.includes("เพศหญิง")) {
        datasets.push({
          label: "ผู้ป่วยหญิง",
          backgroundColor: "rgba(153, 102, 255, 0.6)",
          data: selectedAgeKeys.map(
            (key) => this.ageGroups[`${key}Female`] || 0
          ),
        });
      }
      return {
