    ...
    mounted() {
        ...
        axios.get(`http://localhost:8080/Allpatient`).then((response) => {
            ...
            for (let i = 0; i < this.posts.length; i++) {
                axios.get(`http://localhost:8080/patients/doctor/${response.data[i].doctorId}`).then((res) => {
        ...
        axios.get(`http://localhost:8080/Allformula`).then((response) => {
    ...
    methods: {
        sortPatientInfo() {
            if (this.sortInfo != 'เรียงลำดับตาม HN') {
                ...
                axios.post(`http://localhost:8080/sortInfo`, data).then((response) => {
                    this.posts = response.data[0]
                    for (let i = 0; i < this.posts.length; i++) {
                    axios.get(`http://localhost:8080/patients/doctor/${this.posts[i].doctorId}`).then((res) => {
                        this.posts[i].doctorName = res.data[0].doctorFirstName + " " + res.data[0].doctorLastName
                    }).catch((error) => {
                        console.log(error)
                    })
        ...
        selectedPatient() {
            ...
            axios.post(`http://localhost:8080/selectedPatient`, data).then((response) => {
                this.posts = response.data[0]
                for (let i = 0; i < this.posts.length; i++) {
                    axios.get(`http://localhost:8080/patients/doctor/${this.posts[i].doctorId}`).then((res) => {
                        this.posts[i].doctorName = res.data[0].doctorFirstName + " " + res.data[0].doctorLastName
