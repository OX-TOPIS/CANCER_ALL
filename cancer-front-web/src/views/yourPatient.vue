  ...
  mounted() {
    ...
    axios.get(`https://cancer-api.gadoz.dev/Allpatient`).then((response) => {
      ...
      for (let i = 0; i < this.posts.length; i++) {
        axios
          .get(
            `https://cancer-api.gadoz.dev/patients/doctor/${response.data[i].doctorId}`
          )
          .then((res) => {
            this.posts[i].doctorName =
              res.data[0].doctorFirstName + " " + res.data[0].doctorLastName;
          })
          .catch((error) => {
            console.log(error);
          });
  ...
  methods: {
    ...
    sortPatientInfo() {
      if (this.sortInfo != "เรียงลำดับตาม HN") {
        ...
        axios
          .post(`https://cancer-api.gadoz.dev/sortInfo`, data)
          .then((response) => {
            this.posts = response.data[0];
            for (let i = 0; i < this.posts.length; i++) {
              axios
                .get(
                  `https://cancer-api.gadoz.dev/patients/doctor/${this.posts[i].doctorId}`
                )
                .then((res) => {
                  this.posts[i].doctorName =
                    res.data[0].doctorFirstName +
                    " " +
                    res.data[0].doctorLastName;
                })
                .catch((error) => {
                  console.log(error);
                });
    ...
    goToHis(p) {
      this.$router.push(
        `/DetailPatient/${this.$route.params.userId}/${p.HN}/${p.treatmentId}`
      );
    },
    goToBlood() {
